...
- 表型准确率尚未通过人工参考验证；
- 跨作物迁移已经封存，不得主动扩展。
- 人工复核协作规范与可复现打包脚本位于`experiment/manual_review_system/`。方案C首轮train/val已完成：重复性F1中位数0.933，二维拆分的A/B/C/D总路径数为64/76/63/74；新增路径和三个零路径失败必须先通过平台三独立复核，再打开平台四比较。协作包现含有效域、路线B路径、方案C独立路径和二维拆分对照四个平台；人工判断只用于评价，不作为关键点训练标签。
