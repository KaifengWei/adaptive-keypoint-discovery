...
1. Rater 1第一轮16张已于2026-08-31导出；
2. 原始导出有16个session、35条可测轨迹，图片清单和曲线结构完整；
3. 两条记录存在“轻微遮挡并按规则插值”与“是否插值=否”的字段组合，原始文件已保留，修正前状态为 `pending`；
4. Rater 1第二轮须与第一轮间隔3–7天，且不得查看第一轮轨迹；
5. Rater 2推荐完成16张，最低必须完成全部Core 12；
6. 两轮与第二测量者完成后，先计算intra/inter-rater可靠性、SEM和MDC95，再进行方法盲裁决；
7. 人工GT冻结后才允许解盲并比较Teacher-direct、Student-B和冻结Student-D。
