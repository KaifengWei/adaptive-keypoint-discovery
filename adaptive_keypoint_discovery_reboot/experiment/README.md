...
V4 full-plant基线、路线B和方案C首轮train/val均已完成。方案C在RTX3090完成216张train结构覆盖教师、80轮同种子训练和40张val-only评估；重复性F1中位数为0.933。二维拆分的A/B/C/D总路径数为64/76/63/74，说明局部尺度解码是路径增加的主要来源，新教师则重新分配了终端响应。自动结果仍有`0012`、`0023`、`0031`零路径失败，新增路径也可能包含假枝，必须先完成平台三独立复核，再打开平台四比较。完整证据见`结构覆盖增强方案C首轮训练与二维拆分结果_20260723.md`。在方案C双人复核完成前，不进入人工表型误差、五随机种子、最终消融或test；任何模型命令都不得包含V4 test。
